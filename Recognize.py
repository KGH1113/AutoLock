...
def main(name, id):
    ...
    face_classifier = cv2.CascadeClassifier(os.path.join(crrnt_directory, 'haarcascade_frontalface_default.xml'))
    ...
    while cap.isOpened():
        ...
        faces, msg = recognize(name_id, face_classifier, model, min_accuracy, frame=image)
        ...
        if msg == name_id:
            ...
            for (x, y, w, h) in faces:
                ...
                cv2.rectangle(image, (x,y-base-txt[1]), (x+txt[0], y+txt[1]), \
                            (0,255,255), -1)
                cv2.putText(image, msg, (x, y), cv2.FONT_HERSHEY_PLAIN, 1, \
